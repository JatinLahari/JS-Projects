...
function headerComponent(){
    ...
    head.setAttribute("class","bg-dark d-flex");
    // logo created
    let logo = document.createElement("div");
    logo.setAttribute("style","min-height: 45px; width: 20%; background-color: #272838;");
    logo.setAttribute("class", "d-flex flex-column justify-content-center align-items-center");
    let logoName = document.createElement("small");
    logoName.innerHTML = "<b style='font-size: 20px; color: red'>Shopper</b>"
    logo.appendChild(logoName);
    let tagName = document.createElement("small");
    tagName.innerHTML = "<i class='text-white fw-bold'>One Stop Solution</i>"
    logo.appendChild(tagName);
    head.appendChild(logo);

    // search bar
    let searchBar = document.createElement("div");
    searchBar.setAttribute("style","min-height: 45px; width: 65%; background-color: #F6B9AD;");
    searchBar.setAttribute("class", " d-flex justify-content-center align-items-center");
    let searchInput = document.createElement("input");
    searchInput.setAttribute("placeholder","Search Product");
    searchInput.setAttribute("style","padding: 3px;font-size:20px;min-height:35px;width:90%; border:none; border-radius:10px;");

    let magnify = document.createElement("button");
    magnify.setAttribute("style","border: none; background-color: #F6B9AD; font-size: 24px; font-weight: bold;");
    magnify.setAttribute("class","bi-search d-flex justify-content-center align-items-center text-black");
    searchBar.appendChild(searchInput);
    searchBar.appendChild(magnify);
    head.appendChild(searchBar);

    // sign buttons
    let signBtns = document.createElement("div");
    signBtns.setAttribute("style","min-height: 40px; width: 20%; background-color: #272838;");
    signBtns.setAttribute("class","d-flex justify-content-evenly align-items-center");
    let signIn = document.createElement("small");
    signIn.innerText = "Sign In";
    signIn.setAttribute("class","text-white fw-bold btn bi bi-person-fill");
    signIn.addEventListener("click",()=>{
        signInPage();
    });
    signBtns.appendChild(signIn);
...
function signUpPage(){
    ...
    let signUpHead = document.createElement("div");
    signUpHead.setAttribute("class","bg-warning d-flex justify-content-center align-items-center mt-1 mb-4")
    signUpHead.setAttribute("style","font-size:35px; font-weight:bold; border-radius:14px;");
    signUpHead.innerHTML = "<b>Sign Up</b>";
    form.appendChild(signUpHead);
    // name input
    ...
    submit.setAttribute("class","btn btn-primary mt-3");
    ...
    let transferToSignin = document.createElement("label");
    transferToSignin.innerHTML = "Already have an account? <a onclick='signInPage()' class='fw-bold m-0'>Login</a>";
    form.appendChild(transferToSignin);
...
function signInPage(){
    ...
    inform.setAttribute("style","box-shadow:0px 0px 20px 5px grey; width:30%; margin-top:50px; padding:20px; margin-bottom:100px;");
    let signInHead = document.createElement("div");
    signInHead.setAttribute("class","bg-warning d-flex justify-content-center align-items-center mt-1 mb-4")
    signInHead.setAttribute("style","font-size:35px; font-weight:bold; border-radius:14px;");
    signInHead.innerHTML = "<b>Sign In</b>";
    inform.appendChild(signInHead);

    let nameLabel = document.createElement("label");
    nameLabel.innerHTML = "<b>*Enter Name or email:</b>"
    inform.appendChild(nameLabel);    
    ...
    passwordLabel.innerHTML = "<b>*Enter Password:</b>";
    passwordLabel.setAttribute("class","mt-2");
    inform.appendChild(passwordLabel);
    let passwordInput = document.createElement("input");
    passwordInput.setAttribute("id","passwordInput");
    passwordInput.setAttribute("type","password");
    passwordInput.setAttribute("class","form-control text-success");
    passwordInput.setAttribute("placeholder","Enter Password");
    inform.appendChild(passwordInput);
    let error = document.createElement("small");
    error.setAttribute("id","passwordError");
    error.setAttribute("style","color:red;");
    inform.appendChild(error);

    let loginBtn = document.createElement("button");
    loginBtn.innerHTML = "<b>Login</b>"
    loginBtn.setAttribute("id","loginBtn");
    loginBtn.setAttribute("class","btn btn-primary mt-3");
    loginBtn.setAttribute("style","width:100%;");
    ...
    inform.appendChild(loginBtn);
    main.appendChild(inform);
}
function card_container(data){
    ...
    for(let product of data){
        ...
        col.setAttribute("class","col-md-3 p-2");

        let cardDiv = document.createElement("div");
        cardDiv.setAttribute("style","box-shadow:10px 10px 10px grey; height: 300px;");
        cardDiv.setAttribute("class","d-flex flex-column align-items-center");

        let productImg = document.createElement("img");
        productImg.setAttribute("style","width: 100%; height: 200px;");
        productImg.src = product.thumbnail;
        cardDiv.appendChild(productImg);

        let h6 = document.createElement("h6");
        h6.innerHTML = product.title.substring(0,30);
        cardDiv.appendChild(h6);

        let pPRice = document.createElement("h4");
        pPRice.innerHTML = `<b class='text-success'>${product.price} Rs.</b>`;
...
function viewMoreComponent(product){
    ...
    viewMore.setAttribute("class","container row m-5");
    ...
    mainImage.setAttribute("style","width:100%; height: 450px");
    leftDiv.appendChild(mainImage);

    let multiImagesContain = document.createElement("div");
    multiImagesContain.setAttribute("class","d-flex justify-content-center align-items-center");
    for(let images of product.images){
        let smallImg = document.createElement("img");
        smallImg.src = images;
        smallImg.setAttribute("style","width:33%; height:80px;");
        smallImg.addEventListener("click", ()=>{
            let temp = mainImage.src;
            mainImage.src = smallImg.src;
            smallImg.src = temp;
        });
        multiImagesContain.appendChild(smallImg);
    }
    leftDiv.appendChild(multiImagesContain);
    ...
    pTitle.innerHTML = `${product.title} <b class="text-danger">[${product.brand}]</b><hr>`;
    rightDiv.appendChild(pTitle);

    let pDescription = document.createElement("p");
    pDescription.innerText = product.description;
    rightDiv.appendChild(pDescription);

    let warrantyInformation = document.createElement("p");
    warrantyInformation.innerHTML = `<b>Warranty Information :</b> ${product.warrantyInformation}`;
    rightDiv.appendChild(warrantyInformation);

    let shippingInformation = document.createElement("p");
    shippingInformation.innerHTML = `<b>Shipping Information :</b> ${product.shippingInformation}`;
    rightDiv.appendChild(shippingInformation);

    let returnPolicy = document.createElement("p");
    returnPolicy.innerHTML = `<b>Return Policy :</b> ${product.returnPolicy}`;
    rightDiv.appendChild(returnPolicy);

    let availabilityStatus = document.createElement("p");
    availabilityStatus.innerHTML = `<b>Availability Status :</b> ${product.availabilityStatus}`;
    rightDiv.appendChild(availabilityStatus);

    let rating = document.createElement("p");
    rating.innerHTML = `<b>Rating : </b><span style='font-weight:bolder; font-size: 20px; color:rgb(34, 64, 233);'>${product.rating}/5</span>`;
    rightDiv.appendChild(rating);

    let price = document.createElement("p");
    price.innerHTML = `<b>Price :</b> <del class='text-danger'>${product.price}</del> <span class='text-success fs-5'>${(product.price - (product.price*product.discountPercentage)/100).toFixed(2)} Rs.</span>`;
    rightDiv.appendChild(price);

    let btnDiv = document.createElement("div");
    btnDiv.setAttribute("class","d-flex justify-content-evenly align-items-evenly mt-2");
    rightDiv.appendChild(btnDiv);
    let buyNow = document.createElement("button");
    buyNow.innerText = "Buy Now";
    buyNow.setAttribute("class","btn btn-success");
    buyNow.setAttribute("style","width:35%; font-weight:bold;")
    btnDiv.appendChild(buyNow);
    let addCart = document.createElement("button");
    addCart.innerText = "Add to Cart";
    addCart.setAttribute("class","btn btn-warning");
    addCart.setAttribute("style","width:35%; font-weight:bold;")
    btnDiv.appendChild(addCart);

    main.appendChild(viewMore);
}
function footer(){
